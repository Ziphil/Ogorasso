...
export function getAllPossibleInflections(anatomy: Anatomy): Array<SubstantInflection> | Array<VerbantInflection> {
  const sort = getInflectionSort(anatomy);
  if (sort === "substant") {
    const inflections = [] as Array<SubstantInflection>;
    for (const category of ["base", "adjective"] as const) {
      for (const phoricity of PHORICITIES) {
        for (const gender of GENDERS) {
          for (const caze of CASES) {
            for (const definiteness of DEFINITENESSES) {
              ...
              const inflection = {sort, category, phoricity, gender, case: caze, definiteness, short};
    ...
    const inflections = [] as Array<VerbantInflection>;
      ...
      for (const voice of VOICES) {
        for (const phoricity of PHORICITIES) {
          for (const gender of GENDERS) {
            for (const caze of CASES) {
              for (const definiteness of DEFINITENESSES) {
                const inflection = {sort, category, voice, phoricity, gender, case: caze, definiteness} as const;
                inflections.push(inflection);
              }
            }
          }
